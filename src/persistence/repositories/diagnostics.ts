...
export class DiagnosticArtifactRepository {
  ...
  async insert(input: DiagnosticArtifactInsert): Promise<number> {
    const result = this.ctx.db
      ...
      .returning({ id: diagnosticArtifacts.id })
      .all();
    const row = result[0];
  ...
  async findById(id: number): Promise<DiagnosticArtifactRow | null> {
    const rows = this.ctx.db
      ...
      .where(eq(diagnosticArtifacts.id, id))
      .all();
    const row = rows[0];
  ...
  async listByRun(pipelineRunId: number): Promise<readonly DiagnosticArtifactRow[]> {
    const rows = this.ctx.db
      .select()
      .from(diagnosticArtifacts)
      .where(eq(diagnosticArtifacts.pipelineRunId, pipelineRunId))
      .all();
  ...
  async listBySearch(searchExecutionId: number): Promise<readonly DiagnosticArtifactRow[]> {
    const rows = this.ctx.db
      .select()
      .from(diagnosticArtifacts)
      .where(eq(diagnosticArtifacts.searchExecutionId, searchExecutionId))
      .all();
    return rows.map(rowFromRecord);
  }
  ...
  async listByJob(jobId: number): Promise<readonly DiagnosticArtifactRow[]> {
    const rows = this.ctx.db
      .select()
      .from(diagnosticArtifacts)
      .where(eq(diagnosticArtifacts.jobId, jobId))
      .all();
    return rows.map(rowFromRecord);
  }
  ...
  async listByDiscoveryError(discoveryErrorId: number): Promise<readonly DiagnosticArtifactRow[]> {
    const rows = this.ctx.db
      .select()
      .from(diagnosticArtifacts)
      .where(eq(diagnosticArtifacts.discoveryErrorId, discoveryErrorId))
      .all();
    return rows.map(rowFromRecord);
  }

  async listByExtractionAttempt(
    extractionAttemptId: number,
  ): Promise<readonly DiagnosticArtifactRow[]> {
    const rows = this.ctx.db
      .select()
      .from(diagnosticArtifacts)
      .where(eq(diagnosticArtifacts.extractionAttemptId, extractionAttemptId))
      .all();
    return rows.map(rowFromRecord);
  }
}
