...
The barrel (`src/errors/index.ts`) is the intended public entry point; the sidecar's HTTP error mapper translates `ApplicationErrorJSON` payloads into HTTP responses (mapping `exitCode` to status: `InvalidUsage` -> 400, `MissingRequired` -> 422, `ResourceNotFound` -> 404, `LinkedInBlocked` -> 502, `OpenAIFailure` -> 502, `Fatal` -> 500). The `exitCode` field is the cross-runtime category token; tests assert on it directly.
