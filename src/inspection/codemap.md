...
- **Service layer** — `JobsListService`, `JobsShowService`, `RunsListService`, `RunsShowService` in `services/`. They query repositories, sort via `sortJobListRows`, and assemble the `JobListResult` envelope. `JobsListService.list()` uses a single `preloadPageData(repositories, jobIds)` **PageContext** pre-pass that issues three batched repository queries (filter probes, score probes, extraction attempts) and partitions the results into `ReadonlyMap`s keyed by `jobId`. The per-state mappers then consult the maps synchronously — the page issues a fixed 3 SELECTs regardless of `N` (audit B3-C.1.11). `RunsShowService.computeJobCounts` uses the same pattern (`jobs.findByIds(ids)` + in-memory tally).
